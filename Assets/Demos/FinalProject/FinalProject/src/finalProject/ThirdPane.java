 ...
 * Description: This class creates a GUI for the load file*/
...
public class ThirdPane extends GridPane
{
	...
	public ThirdPane(int rows,int cols, int seltCols, int seltRows, String[][] arr)
	{
		this.rows = rows;
		this.columns = cols;
		this.seltCols = seltCols;
		this.seltRows = seltRows;
		//TO KEEP TRACK FOR GAME
		tracker = new String[arr.length][arr[0].length];
		
		for(int r = 0; r < arr.length; r++)
		{
			for(int c = 0; c < arr[r].length; c++)
			{
				this.tracker[r][c] = arr[r][c];
			}
		}
		...
		if(boss == 0)
		{
			BossNotice bs = new BossNotice();
		}
		else if(printWins == 1)
		{
			MonsterNotice mons = new MonsterNotice();
		}
		else if(printWins == 2)
		{
			TrapNotice trap = new TrapNotice();
		}
		else if(printWins == 3)
		{
			MonsterNotice mons = new MonsterNotice();
	...
	public void menuBar()
	{
		...
		b3.setOnAction(e ->{
			 try {
	                ...
	                this.mainStage.hide();
	                this.st.hide();
	                //ThirdPane pano = new ThirdPane(data.getSetRows(),data.getSetCols(),data.getArr());
	            }
	            catch (Exception ek) {
	...
	private void setSprites()
	{
			...
			for(int i = 0; i < this.sprites.length; i++)
			{
				...
				{
					for(int c = 0; c < this.tracker[0].length; c++)
					{
						if(this.tracker[r][c] == this.sprites[i])
						{
							this.add(this.images[i], c, r);
						}
					}
				}
			}
		...
		if(this.rows == 5 && this.columns == 5)
		{
			...
			for(int r = 0; r < this.tracker.length; r++)
			{
				for(int c = 0; c < this.tracker[r].length; c++)
				{
					...
					else if(this.tracker[r][c].equals(this.sprites[5]))
					{
						this.tracker[r][c] = "-";
					}
				}
	...
	{
		this.tracker[this.bossRows][this.bossCols] = "-";
		this.getChildren().remove(this.images[0]);
		boolean moved = false;
		int side  = r.nextInt(4);
		while(!moved)
		{
			if((this.bossCols + 1 <= this.tracker[0].length - 1) && (this.tracker[this.bossRows][this.bossCols + 1] == "-") && side == 0 )
			{
				this.bossCols++;
				this.tracker[this.bossRows][this.bossCols] = this.sprites[0];
				this.add(this.images[0],this.bossCols, this.bossRows);
				moved = true;
			}
			if(this.bossCols - 1 >= 0 && (this.tracker[this.bossRows][this.bossCols - 1] == "-") && side == 1)
			{
				this.bossCols--;
				this.tracker[this.bossRows][this.bossCols] = this.sprites[0];
				this.add(this.images[0],this.bossCols, this.bossRows);	
			...
			if(this.bossRows + 1 <= this.tracker[0].length - 1 && (this.tracker[this.bossRows + 1][this.bossCols] == "-") && side  == 2)
			{
				this.bossRows++;
				this.tracker[this.bossRows][this.bossCols] = this.sprites[0];
				this.add(this.images[0],this.bossCols, this.bossRows);
			...
			if(this.bossRows - 1 >= 0 && (this.tracker[this.bossRows - 1][this.bossCols] == "-") && side == 3)
			{
				this.bossRows--;
				this.tracker[this.bossRows][this.bossCols] = this.sprites[0];
				this.add(this.images[0],this.bossCols, this.bossRows);
